...
import { encodeCursor, decodeCursor, ghostTags } from "../../util";

const POST_FILTER = "tag:hash-bookmark";
const HARD_LIMIT = 15;
...
comments: String
*/
const post2Bookmark = (post) => ({
  id: post.id,
  title: post.title || null,
  publishedAt: post.published_at,
  url: post.canonical_url,
  tags: ghostTags(post.tags),
  comments: post.html,
});
...
export default async function bookmarks(
  ...
  const edges = posts.map((post, idx) => {
    const clean = post2Bookmark(post);
    return {
      node: clean,
      cursor: encodeCursor({
        type: "Bookmark",
