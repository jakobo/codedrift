...
const Logo = forwardRef<HTMLDivElement, LogoProps>(
  (
    ...
    const ratio =
      (Array.isArray(text) && text.length === 0) || text === false
        ? VIEWBOX.ICON
        : VIEWBOX.WORDMARK;
...
const CD: React.FC<CDProps> = ({ left, right }) => (
...
const Drift: React.FC<DriftProps> = ({ text }) => (
...
interface IconOnlyProps extends SVGAttributes<SVGElement> {
  left: string;
  right: string;
}
const IconOnly: React.FC<IconOnlyProps> = ({ left, right, ...props }) => (
  <svg
...
interface FullLogoProps extends SVGAttributes<SVGElement> {
  left: string;
  right: string;
  text: string[] | false;
}
const FullLogo: React.FC<FullLogoProps> = ({ left, right, text, ...props }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
